...
std::string getFunctionName(Dwarf_Debug dbg, Dwarf_Die func_die, bool demangle, std::string def_val) {
	Dwarf_Error error = nullptr;

	// Linkage name(mangled name)
	Dwarf_Attribute attr1 = nullptr;
    if (dwarf_attr(func_die, DW_AT_linkage_name, &attr1, &error) == DW_DLV_OK) {
		auto guard = make_scope_exit([&]() { dwarf_dealloc(dbg, attr1, DW_DLA_ATTR); });
		return demangle ? demangleName(getAttrStr(attr1, def_val)) : getAttrStr(attr1, def_val);
	} 
	Dwarf_Attribute attr2 = nullptr;
    if (dwarf_attr(func_die, DW_AT_MIPS_linkage_name, &attr2, &error) == DW_DLV_OK) {
		auto guard = make_scope_exit([&]() { dwarf_dealloc(dbg, attr2, DW_DLA_ATTR); });
		return demangle ? demangleName(getAttrStr(attr2, def_val)) : getAttrStr(attr2, def_val);
	}
	Dwarf_Attribute attr3 = nullptr;
    if (dwarf_attr(func_die, DW_AT_HP_linkage_name, &attr3, &error) == DW_DLV_OK) {
		auto guard = make_scope_exit([&]() { dwarf_dealloc(dbg, attr3, DW_DLA_ATTR); });
		return demangle ? demangleName(getAttrStr(attr3, def_val)) : getAttrStr(attr3, def_val);
	}
	...
	if (dwarf_attr(func_die, DW_AT_specification, &attr, &error) == DW_DLV_OK) {
		auto guard = make_scope_exit([&]() { dwarf_dealloc(dbg, attr, DW_DLA_ATTR); });
		...
		if (ref_die_ref != MAX_DWARF_OFF) {
			...
			if (getDieFromOffset(dbg, ref_die_ref, ref_die)) {
				auto die_guard = make_scope_exit([&]() { dwarf_dealloc(dbg, ref_die, DW_DLA_DIE); });
...
std::pair<std::string, Dwarf_Unsigned> getFileNameAndLineNumber(Dwarf_Debug dbg, Dwarf_Die cu_die, Dwarf_Addr pc, std::string def_val1, Dwarf_Unsigned def_val2) {
	...
	auto line_context_guard = make_scope_exit([&]() { dwarf_srclines_dealloc_b(line_context); });
	...
	if (dwarf_srclines_from_linecontext(line_context, &line_buf, &line_count, &error) != DW_DLV_OK) {
		DWARF_ERROR(getDwarfError(error));
		return result;
	}
