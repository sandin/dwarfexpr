 ...
 */
DwarfFile::DwarfFile(string fileName) :
		m_CUs(this),
		m_lines(this),
		m_functions(this),
		m_types(this),
 ...
 */
bool DwarfFile::loadFile(const DwarfFilter& filter)
{
...
#ifdef OS_WINDOWS
	if ((m_fd = _open(m_fileName.c_str(), _O_RDONLY, 0)) < 0)
#else
	if ((m_fd = open(m_fileName.c_str(), O_RDONLY, 0)) < 0)
#endif
	{
		DWARF_ERROR("File: \"" << m_fileName << "\" can not be opened.");
		return false;
	}

	m_res = dwarf_init_b(m_fd, DW_GROUPNUMBER_ANY, errHand, errArg, &m_dbg, &m_error);

	if (m_res == DW_DLV_NO_ENTRY)
	{
		DWARF_WARNING("File: \"" << m_fileName << "\" has no DWARF information.");
		return false;
	}
...
bool DwarfFile::loadDIE(Dwarf_Die die, DwarfBaseElement* &parent, int lvl, const DwarfFilter& filter)
{
	...
	AttrProcessor ap(m_dbg, die, this, 0, 0); // FIXME: version
	...
	switch (tag)
	{
		...
		{
			DwarfFunction *func = m_functions.loadAndGetDie(die, lvl);
			parent = func;
