...
DwarfLine *DwarfLineContainer::loadAndGetDie(Dwarf_Die cu_die, unsigned)
{
	...
	//m_res = dwarf_srclines_b(cu_die, &lineBuf, &lineCnt, &m_error);
	m_res = dwarf_srclines_b(cu_die, &lineversion, &table_count, &line_context, &m_error);
	...
	if (table_count == 0) {
		...
		m_res = dwarf_srclines_from_linecontext(line_context, &lineBuf,&lineCnt, &m_error);
	} else if (table_count == 2) {
		m_res = dwarf_srclines_two_level_from_linecontext(line_context, &lineBuf, &lineCnt,
			&linebuf_actuals,&linecount_actuals, &m_error);
	}
