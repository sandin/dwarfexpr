...
class DwarfCUContext {
 public:
  DwarfCUContext(Dwarf_Debug in_dbg, Dwarf_Addr in_pc, int in_is_info,
                 int in_in_level, int in_cu_number)
      : dbg(in_dbg),
        target_pc(in_pc),
        cu_die(nullptr),
        func_die(nullptr),
...
class DwarfSearcher {
 public:
  ...
  bool searchFunction(Dwarf_Addr pc, Dwarf_Die* out_cu_die,
